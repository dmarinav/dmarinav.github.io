...
Finally, I performed a random over-sampling technique. Since more Titanic passengers died than survived, the number of observations belonging to this class was substantially higher than those belonging to the other class. Out of all minority over-sampling techniques, I chose Random Oversampler to combat imbalanced classes. From my experience, it tends to yield higher model accuracy scores. To evaluate and tune my model, I conducted [nested cross-validation](https://chrisalbon.com/machine-learning/nested_cross_validation.html)since it is the best way to evaluate a model when a data set is small. This technique allows to avoid a model's overfitting (Cawley, & Talbot, 2010). 

After performing all these techniques, I obtained a 99.08% accuracy score on my training set, 98.64% on my test set, and 98.91% accuracy on my complete data set. Below are the confusion matrices for training, test, and complete data sets.
