...
Inertia values shown above, measure of how internally coherent clusters are, indicate that the PCA-reduced clusters contain less distortion that the original clusters do, and the distortion decreases with a number of clusters. Overall, the high inertia values were indicative of a high distortion in the clusters (ideally, the less this number is the less the distortion is), indicating that clusters’ cohesiveness might be too high.  Since [Euclidean distance](https://en.wikipedia.org/wiki/Euclidean_distance) tends to become inflated (a so-called [curse of dimensionality](https://en.wikipedia.org/wiki/Curse_of_dimensionality)), running a PCA prior to k-means alleviates this problem.
...
As seen above, the hierarchical clustering with the complete linkage forms two clusters – just like in the case of the k-means analysis. On the other hand, the hierarchical clustering with the average linkage forms one cluster and one outlier – just like in the case of the DBSCAN analysis. In reality, it forms two clusters, but because the second cluster consists only of one observation, the average linkage method surves as an outlier detector in this case. 
